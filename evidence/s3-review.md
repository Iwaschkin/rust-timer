...
| Host | Windows 11 x86_64 local; CI run <https://github.com/Iwaschkin/rust-timer/actions/runs/36091436371> did not start (account billing) |
...
| Hosted CI | not started: "recent account payments have failed or your spending limit needs to be increased" | 2 to 5 | the CI run above |
...
- The only platform `cfg` is C07's argument builder in `tests/cli.rs`: the Windows branch ran here, the Unix branch has not run.
...
- No hosted run for this slice: Linux, including C07's Unix branch, has not run.
